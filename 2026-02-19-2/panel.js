            ...
            ${securityLogs.slice(0, 50).map(log => `
                <div style="margin-bottom: 30px; border-left: 5px solid #dc3545; padding-left: 15px;">
                    ...
                    <div style="margin: 5px 0;">
                     상태: ${log.status} | 응답시간: ${log.latency} ms
                    </div>
...
// DevTools 패널에서 다운로드 안정화
const a = document.createElement('a');
a.href = url;
a.download = `Security_Report_${Date.now()}.html`;

document.body.appendChild(a); // ⭐ 중요 (없으면 두번째부터 실패 가능)
a.click();
a.remove();

// 메모리 정리 (없으면 다음 다운로드 실패 가능)
...
/* =================================================
   Advanced Security Scanner (Addon)
   ================================================= */
...
/* =====================================================
   False Positive Reduction Layer (Non-invasive)
   - 기존 탐지 로직 수정 없음
   - 오탐만 후처리로 걸러냄
===================================================== */

(function () {
  const FP_FLAG = 'data-fp-checked';
  ...
  const SAFE_VALUE_PATTERNS = [
    /^[A-Za-z0-9+/=]{20,}$/ // base64 payload
  ];

  function looksLikeBase64(str) {
    return SAFE_VALUE_PATTERNS.some(r => r.test(str)) && str.length % 4 === 0;
  }

  function shouldDowngrade(text) {
    if (!text) return false;

    if (SAFE_CONTEXT_PATTERNS.some(r => r.test(text))) {
      return true;
    }

    const tokens = text.match(/[A-Za-z0-9_\-\.]{20,}/g) || [];
    return tokens.every(t => looksLikeBase64(t));
  }

  function injectFalsePositiveAnalysis(card) {
    if (card.hasAttribute(FP_FLAG)) return;

    const text = card.innerText || '';
    if (!text.includes('API Key (Generic)')) return;

    if (!shouldDowngrade(text)) return;

    card.setAttribute(FP_FLAG, 'true');

    const block = document.createElement('div');
    block.style.marginTop = '8px';
    block.style.paddingTop = '6px';
    block.style.borderTop = '1px dashed #4ade80';
    block.style.fontSize = '12px';
    block.style.lineHeight = '1.5';
    block.style.color = '#4ade80';

    block.innerHTML = `
      <strong>[오탐 가능성 분석]</strong><br>
      🟢 <strong>Low Risk</strong> (자동 판단)<br>
      - 이유: 알려진 서비스 컨텍스트 또는 분석용 데이터 패턴<br>
      - 조치: 보안 위협으로 분류하지 않아도 됨
    `;

    card.appendChild(block);
    card.classList.remove('has-security-issue');
  }

  const observer = new MutationObserver(() => {
    document
      .querySelectorAll('.log-item')
      .forEach(injectFalsePositiveAnalysis);
  });
